...
/**
 * Specifies whether the axis auto-calculates its range when the chart reloads
 * its data.
 *
 * By default, this property is set to `YES`.
 */
...
/**
 Attempts to set the current visible range `range` to a range with the given
 minimum and maximum values.

 Given any restrictions on setting the range, such as
 `allowPanningOutOfMaxRange` etc, this method will attempt to set the current
 axis range.
 This implementation allows you to explicitly set whether to animate the
 transition to the new range or not.
